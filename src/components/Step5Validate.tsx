...
export const Step5Validate: React.FC<Step5Props> = ({
  job,
  onValidate,
  isValidating,
}) => {
    ...
    <div className="space-y-6">
          ...
          <div>
            <div className="flex items-center space-x-2">
              ...
              <h2 className="text-lg font-bold text-stone-900">
                Step 5: Validate Output M4B (FFprobe Verification)
              </h2>
            </div>
            <p className="text-sm text-stone-600 mt-1 max-w-3xl">
              ...
              validates that all chapters are properly embedded and seekable, and audits Audiobookshelf metadata tags (Narrator in Composer tag &amp; cover art).
            </p>
          </div>
      ...
      {report ? (
        <div className="space-y-6">
          ...
          {/* Dual Verification Panels: Metadata & Chapters */}
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            {/* Left: Metadata Integrity Audit */}
            <div className="lg:col-span-5 bg-white rounded-xl border border-stone-200/80 shadow-xs overflow-hidden">
              <div className="p-4 border-b border-stone-200 bg-stone-50 flex items-center justify-between">
                <h3 className="font-semibold text-sm text-stone-800 flex items-center space-x-2">
                  <ShieldCheck className="w-4 h-4 text-amber-600" />
                  <span>Audiobookshelf Tag Verification</span>
                </h3>
              ...
              <div className="p-4 space-y-3 text-xs">
                <div className="flex justify-between py-1 border-b border-stone-100">
                  <span className="text-stone-500">Book Title (\xa9nam):</span>
                  <span className="font-semibold text-stone-900 truncate max-w-[180px]">{meta?.title || job.name}</span>
                </div>
                <div className="flex justify-between py-1 border-b border-stone-100">
                  <span className="text-stone-500">Author (\xa9ART):</span>
                ...
                <div className="flex justify-between py-1 border-b border-stone-100 bg-amber-50/50 p-1.5 rounded">
                  <span className="text-amber-900 font-semibold flex items-center space-x-1">
                    ...
                    <span>Narrator (\xa9wrt Composer):</span>
                  </span>
                ...
                <div className="flex justify-between py-1 border-b border-stone-100">
                  <span className="text-stone-500">Series & Part:</span>
                  <span className="font-medium text-stone-900">
                    {meta?.series ? `${meta.series}${meta.seriesSequence ? ` #${meta.seriesSequence}` : ''}` : 'None'}
                  </span>
                </div>
                <div className="flex justify-between py-1 border-b border-stone-100">
                  <span className="text-stone-500">Genres:</span>
                  <span className="font-medium text-stone-900">
                    {meta?.genres && meta.genres.length > 0 ? meta.genres.join(', ') : 'Audiobook'}
                  </span>
                </div>
                <div className="flex justify-between py-1 border-b border-stone-100">
                  <span className="text-stone-500">Edition / Advisory:</span>
                  <span className="font-medium text-stone-900">
                    {meta?.abridged ? 'Abridged' : 'Unabridged'} • {meta?.explicit ? 'Explicit' : 'Clean'}
                  </span>
                </div>
                <div className="flex justify-between py-1 border-b border-stone-100">
                  <span className="text-stone-500 flex items-center space-x-1">
                    <Image className="w-3.5 h-3.5 text-stone-400" />
                    <span>Cover Art (covr atom):</span>
                  </span>
                  <span className={`font-semibold ${meta?.cover ? 'text-emerald-700' : 'text-stone-400'}`}>
                    {meta?.cover ? `Embedded (${meta.cover.width || 1400}×${meta.cover.height || 1400}px)` : 'None'}
                  </span>
                </div>
              </div>
            </div>

            {/* Right: Embedded Chapters Verification List */}
            <div className="lg:col-span-7 bg-white rounded-xl border border-stone-200/80 shadow-xs overflow-hidden">
              <div className="p-4 border-b border-stone-200 bg-stone-50 flex items-center justify-between">
                ...
                <span className="text-xs text-stone-500 font-mono">
                  {job.chapters.length} chapters verified
                </span>
              </div>
        ...
        <div className="p-12 text-center bg-white rounded-xl border border-stone-200/80 shadow-xs space-y-3">
          ...
          <p className="text-xs text-stone-500 max-w-md mx-auto">
            {job.outputM4b
              ...
              : 'Complete Step 4 to build your M4B audiobook before running validation.'}
          </p>
        </div>
      )}
    </div>
